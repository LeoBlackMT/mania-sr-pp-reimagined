...
//!
//! Faithful port of the research reference `mania_surface_research/mods.py` (the single
//! source of mod semantics on the Python side). The module owns
//!
...
//!   windows they produce (see `crate::windows`), because a mod-name multiplier can be
//!   bypassed by `DA` setting the same OD.
//! * `NF` is not a flat `0.75`: it is derived from the *failure risk* of the score
//!   (effective HP, per-judgement health deltas, EZ's extra lives), converging to the
//!   official `0.75` only when the score really would have failed without the mod.
//! * `HT` / `DC` have **no** PP multiplier. The `0.3` in lazer is a *score* multiplier;
//!   the rate change is already carried by the SR variant (`rate`), so multiplying again
//!   would price the speed twice.
...
///
/// The order is the order of [`Counts`]: `[n320, n300, n200, n100, n50, miss]`, i.e.
...
///
/// Note that `HR`, `DA`, `CL`, `V2`, the key-count mods `1K`/`2K`/`3K`/`10K`, `RD`, `DS`,
/// `IN`, `CS`, `HO`, `NR`, `WU`, `WD` and `AS` are **unranked**; this project still models
/// their mathematical effect (model coverage, not ranked coverage).
...
enum WindowEffect {
    ...
    Easy,
    /// `DA`: does not scale the windows, it *overrides the OD* (the OD value is supplied by
    /// the caller through `windows::WindowOptions::da_od`).
    DifficultyAdjust,
...
struct ModInfo {
    ...
    acronym: &'static str,
    /// Official flat PP multiplier (`1.0` = none). `NF`/`EZ`/`HR` are excluded from the
    /// product by [`mod_effects`] because they have their own mechanisms.
    pp_mult: f64,
    ...
    rate: Option<f64>,
    /// Forced column count. Only has an effect on std -> mania converts; on native mania
    /// maps the key mods are a no-op (`ManiaKeyMod` returns early for the current ruleset).
    keys: Option<i32>,
    /// Long-note effect (`HO` rewrites holds as plain notes, `NR` removes the tail release).
    ///
    /// Kept for fidelity with the reference's mod table, but **not part of the frozen API**:
    /// the Rust `ModEffects` exposes neither `hold_off` nor `no_release`, so nothing consumes
    /// it here yet.
    #[allow(dead_code)]
    ln: LnEffect,
...
/// The mod table. The order is the reference's `MODS` dict order and is significant:
/// [`ModSet::parse`] looks acronyms up in this order among equal-length ones, exactly like
/// Python's stable `sorted(MODS, key=len, reverse=True)`.
///
/// `rustfmt` is skipped so that the table keeps one mod per line, mirroring the reference's
/// dict (it would otherwise expand to nine lines per row).
#[rustfmt::skip]
static MODS: &[ModInfo] = &[
...
/// `HR`'s window difficulty multiplier (`ManiaModHardRock.HIT_WINDOW_DIFFICULTY_MULTIPLIER`):
/// the windows are divided by it.
...
///
/// Python computes `sorted(MODS, key=len, reverse=True)` once at import time; the sort is
/// stable, so equal-length acronyms keep the table order. Longest-first is what makes
/// `"10K"` win over `"1K"` in an unseparated mod string.
...
/// A parsed mod set.
///
/// The acronym list is what [`ModSet::parse`] found, in the reference's lookup order
/// (longest acronym first, table order for ties) — **not** the order in the input string.
/// `ModSet::parse("7K+DT").acronyms == ["DT", "7K"]`, exactly like the Python reference.
...
impl ModSet {
    ...
    ///
    /// Matching is **substring based** (case-insensitive), because this project consumes
    /// two encodings: the osu! API writes `DT+MR`, the ppysb report writes `EZDTV2` with no
    /// separators at all. A token split would silently drop every mod of the second kind.
    ///
    ...
    ///
    /// The reference tests membership with `name.upper() in mods.upper()` on the raw string.
    /// For every acronym of the table this is equivalent to membership in the parsed list
    /// (a known acronym is found by parsing if and only if it occurs as a substring), which
    /// is why the parsed set is enough here.
    ...
    ///
    /// The order follows the lookup order of [`ModSet::parse`], so this does **not**
    /// round-trip the input spelling of an unseparated string; it round-trips the mod *set*.
...
pub struct ModEffects {
    /// Window difficulty multiplier as the game defines it: `HR` 1.4, `EZ` 1/1.4, else 1.0.
    /// The windows are `base * (speed / difficulty_multiplier)`; `DA` and `CL` do not appear
    /// here (see `crate::windows`).
    ...
    /// OD overridden by `DA`.
    ///
    /// The reference takes this from a function argument (`mod_effects(mods, da_od)`), not
    /// from the mod string; the frozen Rust signature has no such argument, so this mirrors
    /// the reference's default and stays `None`. The DA OD is threaded through
    /// `windows::WindowOptions::da_od` instead.
    pub da_od: Option<f64>,
}

/// Aggregate every mod of a set into its effects (the single entry point).
///
/// `NF`/`EZ`/`HR` are deliberately kept out of `flat_pp_mult`: `NF` goes through the failure
/// risk model ([`nf_multiplier`]) and `EZ`/`HR` through the judgement windows.
pub fn mod_effects(m: &ModSet) -> ModEffects {
    ...
    for acronym in &m.acronyms {
        // The reference skips acronyms it does not know; `parse` never produces any, but the
        // acronym list is public, so a hand-built set must not panic either.
    ...
    // HR and EZ are mutually exclusive in the game; when both are present the reference
    // resolves the conflict in favour of HR and applies *both* HP multipliers.
...
///
/// The spec export only carries the *tunable* upper clamp (`nf.hp_max`); the lower bound is
/// a fact about the health domain.
const HP_MIN: f64 = 0.0;

/// Python's `min(hi, max(lo, v))` clamp.
///
/// Kept instead of `f64::clamp` because the two differ for NaN: Python's `max`/`min` never
/// select a NaN *second* operand, so the reference's clamp collapses NaN to the lower bound,
/// while `f64::clamp` propagates it. Rust's `f64::max`/`f64::min` ignore a NaN operand in
/// the same way, so writing the clamp in the reference's order reproduces it exactly.
...
///
/// The curve is the *risk-domain exponential* the research specification chose over a
/// literal exponential decay: `g(x) = (e^(k*x) - 1) / (e^k - 1)` with `x = hp/10`, so the
/// slope *grows* with HP (HP 8 and 9 are the knees where the risk of failing jumps) while
/// the endpoints stay exact: HP 0 -> `nf.mult_max` (0.98, NF always keeps a small price)
/// and HP 10 -> `nf.mult_min` (0.75, the official flat anchor).
///
/// Note the deliberate consequence of the exponential form: HP 9 is 0.8359 rather than the
/// 0.855 of the piecewise anchors (the form cannot hit four anchors at once). The spec's
/// `nf.curve` switches to the exact piecewise form if that is ever wanted.
...
// Per-judgement health deltas of `ManiaHealthProcessor.GetHealthIncreaseFor`
// (positive = healing, negative = damage). Mania has no passive drain.
...
///
/// `is_ln` selects the hold variant of a miss: a hold head/tail only costs half of what a
/// plain note costs (`HEALTH_MISS_LN`). The parameter is the HP *after* [`effective_hp`]
/// (`EZ` halves it, `HR` boosts and caps it) — the reference takes the same effective value.
...
///
/// Mirrors the reference's `-health_delta(...) * total` *including its rounding*: the
/// reference first averages the per-judgement deltas over the note count and then multiplies
/// the average back by it, so the result can differ from the raw sum by a last-bit rounding.
/// The miss blend is kept as written there: `ln_hold_share` attributes half of its share to
/// the half-damage hold variant, `0.0075 * (1 - 0.5 * share) + 0.00375 * (0.5 * share)`.
...
///   missing HP data degrades to the official anchor instead of silently cancelling the
///   penalty,
/// * otherwise the risk model below (`0.98 - (0.98 - f_hp) * risk`).
///
/// `hp` is the map's raw HP drain rate (not yet mod-adjusted); `ln_hold_share` is the share
/// of the judgements that are hold heads/tails (see [`ln_hold_share_from_ratio`]).
...
///
/// Not part of the frozen public API: the reference exposes it as `mods.nf_multiplier`, but
/// its Rust counterpart takes `Option<f64>` for the HP and therefore has to carry the
/// "no NF at all" / "HP unknown" handling of `pp_formula.nf_factor`. Keeping the private half
/// separate keeps both behaviours in one place.
///
/// `risk` asks a threshold question rather than "how badly did you play": a No-Fail score is
/// a *censored sample* — runs that would have failed only exist in the data because NF was
/// on. So the penalty interpolates between "NF did nothing" (`risk = 0`, net healing over
/// the whole run -> `mult_max`) and "NF saved you" (`risk >= 1`, the run is worth only the
/// HP curve's price -> `f_hp`), with a smoothstep in between. The middle band is
/// deliberately narrow (`nf.risk_r0`/`risk_r1`) so the model does not double-count the score
/// accuracy curve.
///
/// The consumption is a **reconstruction**, not a measurement: score data carries no health
/// telemetry, so it is simulated from the map HP, the judgement distribution and the
/// per-judgement deltas, assuming judgements arrive at a uniform rate. Clustered misses
...
/// sunny's `ln_ratio` (hold objects / total objects) -> share of *judgements* that are hold
/// heads or tails.
///
/// A hold contributes two judgement slots (head and tail) and a circle one, so
/// `share = 2H / (C + 2H) = 2 * ln_ratio / (1 + ln_ratio)`. The share only enters the No-Fail
/// model, where a missed hold head/tail costs half of a plain-note miss.
///
/// The ratio is clamped to 1.0 first (a ratio above 1 is not representable as
/// holds/notes, but a caller may pass one) and a non-positive ratio yields 0.0.
