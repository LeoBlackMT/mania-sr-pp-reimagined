...
//!
//! Instead of fusing every difficulty signal into one number, the algorithm keeps the three
//! operationally distinct channels apart and fuses them at pricing time:
//!
//! ```text
//! R = sunny star rating of the map with every hold rewritten as a tap ("rice")
//! L = sunny star rating of the full map - R
//!
...
//! ```
//!
//! Each mechanism and its tunables (LN weight anchors, cross-column modulation, accuracy gamma,
//! chord coupling, No-Fail curve, channel volumes) are documented in [`pp`], [`keys`], [`mods`] and
//! [`windows`]; the values themselves come from `mania-pp-spec`, which is exported from the Python
//! research reference.
//!
...
pub fn channels(prepared: &Prepared, counts: &Counts) -> Channels {
    ...
    // w_ref is the *same map without mods*, so the map's own OD is not priced a second time
    // (sunny already contains it through its window parameter).
