...
//!
//! Codexxy is the name its author gives the algorithm; the technique it adds on top of sunny is a
//! **map-based timing surface**, which is why this project's older notes call it "surface". The
//! dataset, the site and this module all use the name **Codexxy**.
//!
...
/// The score state the upstream calculation consumes (kept here so the timing helpers below can
/// recompute the shared pass on their own).
#[allow(dead_code)]
...
///
/// Prefer [`pp_from`] together with a single [`crate::sunny::performance`] call when pricing a score
/// for several columns; this convenience wrapper pays for the shared pass on its own.
