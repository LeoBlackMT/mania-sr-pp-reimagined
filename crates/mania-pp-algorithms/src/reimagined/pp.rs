//! The reimagined PP formula: the 305-weighted accuracy curve, the sunny "rebirth" body, the
//! window-derived accuracy channel and the three-channel combination.
//!
//! Ported one-to-one from `pp_formula.py`. Everything tunable is read from
//! [`mania_pp_spec::spec`]; only the *shape* of the formulas (band boundaries, the `4.5`
//! numerator and the `0.9^20` denominator term of the accuracy curve) is written here.
//!
//! # The pricing pipeline
//!
//! ```text
//! R, L      -> effective_star():  (R_SCALE * R + w * L) * (1 + star_key_boost(keys))
//! eff_star  -> sunny_rebirth_pp(): 9.8 * max(star - 0.15, 0.05)^2.2 * proportion(acc)
...
//! ```
//!
//! `nf_factor` is applied **inside** `sunny_rebirth_pp`, exactly once — reading the pipeline
//! as "…then multiply by nf_factor, then by accuracy_factor" must not lead to applying it
//! twice.
...
///
/// The order is also the order of [`crate::reimagined::mods::Judgement`] (best to worst) and the order of
/// the 305 weights used by [`custom_accuracy`].
...
/// Python's `max(a, b)`: a NaN **first** operand propagates, whereas Rust's `f64::max`
/// returns the other operand. Only used where the reference puts the value first.
...
///
/// The weights are `320 -> 305`, `300 -> 300`, `200 -> 200`, `100 -> 100`, `50 -> 50`,
/// miss `0` (osu!'s "305 score"), so a score can only reach 1.0 with all 320s.
...
/// The four anchors of the reshaped accuracy curve: `p98`, `p99`, `p995` and the tail
/// coefficient `C` of `1 - C * (1 - acc)^k_tail`.
///
/// Computed once and cached, like the reference's lazy `_acc_anchor()`. The band widths
/// (`0.02`, `0.01`) are the reference's literals: they describe the *shape*, they are not
/// knobs, which is why they do not appear in the exported spec.
fn acc_anchors() -> &'static (f64, f64, f64, f64) {
    ...
    ANCHORS.get_or_init(|| {
        ...
        // The 0.98..0.99 band keeps only `g98` of the gain the original curve had there:
        // that band is where high-accuracy scores used to be overestimated.
...
/// Variation (stream pattern variety) -> multiplier: a logistic between `variety_lo` and
/// `variety_hi` with its midpoint at `variety_mid`.
...
///
/// The sigmoid maps the scalar to a weight in `[base, base + amp]`, which is then blended
/// between the two terms of `2 * acc^k - 1` and `2 - 2 * acc^k`. In this project the three
/// channel price always passes `acc_scalar = 1.0` (the accuracy channel is carried by
/// [`accuracy_factor`]), so this is the "old" sunny knob kept for the comparison baselines.
...
///
/// A negative star would take the square root of a negative number: the reference raises
/// `ValueError` there, this port yields NaN (the input is outside the documented domain).
...
///
/// Measured values (`chord.typical_measured`: 4K 1.471, 6K 1.812, 7K 2.109) win; every other
/// key count is extrapolated linearly from the measured 4K value with the measured slope
/// (`chord.slope_per_key`), floored at 1.0 (no chords at all).
...
/// Mean chord -> timing-sigma amplification (`1.0` = the 4K anchor; `> 1` = harder to stay
/// accurate). Diagnostic only: it feeds the expected-loss model, never the price.
...
/// Mean chord -> accuracy *pricing* adjustment (a weak coupling, deliberately about a seventh
/// of the sigma version: the full timing evidence cannot enter the price without moving the
/// overall calibration water level).
...
///
/// `mean_chord` is the map's measured mean simultaneous press count; when it is absent the
/// typical chord of that key count is used ([`typical_chord`]), which degrades the coupling
/// to "key count only". The reference deletes its former per-key hard-coded table precisely
/// because this derivation replaced it.
...
///
/// `ln_gap_cross = None` means "no structural information": the modulation is then exactly
/// 1.0 and the feeling anchors are reproduced bit for bit.
...
///
/// ```text
/// ratio  = perfect_window / w_ref
/// factor = clamp(ratio^(-gamma) * key_acc_boost(keys, mean_chord), clamp_lo, clamp_hi)
/// gamma  = gamma_wide (ratio > 1) | gamma_tight (ratio < 1)
/// ```
///
/// `w_ref` **must be the same map without mods** (`windows_for(od, "")`), not an absolute
/// constant: the map's own OD is already priced by the star rating, so a fixed reference would
/// charge OD twice (a NM score's factor would climb from 0.50 at OD 2 to 1.00 at OD 8). With
/// the map-relative reference, an NM score is exactly 1.0.
///
/// Because `HR`/`EZ`/`DA` all reach the OD and the window through the *same* path (see
/// [`crate::windows`]), a mod-name multiplier is neither needed nor sufficient — `DA` can
/// reproduce an EZ window exactly, so a name-based discount could be bypassed.
...
///
/// `1.0` without `NF`, the official flat `0.75` when the map's HP is unknown, and the
/// risk model otherwise.
...
/// # Why `Option`
///
/// The reference returns `None` when the star, the variety or the accuracy scalar is missing.
/// An `f64` cannot be absent, so the "missing" case is expressed as **NaN** here: a NaN star,
/// variety or scalar returns `None`, which is what a caller with absent difficulty attributes
/// needs (the CLI reports "no pp" rather than a meaningless number). A zero proportion (an
/// accuracy at or below the gate) is *not* missing and returns `Some(0.0)`.
///
/// The multiplication order of the reference is preserved term by term; the `nf_factor` is
/// applied here, once, and not again by the three-channel combination.
...
///
/// The last step is the A-channel volume knob: `ACC_WEIGHT = 1` is the current behaviour,
/// `0` switches the accuracy channel off entirely, and values above 1 amplify it. It only
/// scales the *window-derived* factor — the score's own accuracy curve
/// ([`performance_proportion`]) is never touched by it.
///
/// A missing difficulty attribute (NaN `R`/`L`/`variety`, see [`sunny_rebirth_pp`]) yields NaN,
/// the `f64` spelling of the reference's `None`.
#[allow(clippy::too_many_arguments)]
...
/// Normalized "loss" of a judgement distribution (the expected-loss signal of the accuracy
/// diagnostics, ported from the rust fork's per-judgement timing adjustment).
///
/// Each judgement contributes the amount by which it pulls the accuracy below 1.0, weighted by
/// an asymmetric penalty (`1.0, 1.1, 1.2, 1.4, 1.8, 2.6`). Unlike 305 accuracy, this is
/// *unsaturated* near the top, which is why it can drive an accuracy channel that OD, mods and
/// key count all feed into.
...
///
/// Compares the player's judgement distribution with the map's expected one: a player who
/// loses less than expected gets a multiplier above 1, one who loses more gets below 1.
/// `1.0` when either distribution is empty.
...
/// ```
///
/// `loss` (the first parameter) is the map's **expected timing loss** — the reference calls it
/// `timing_loss`. The parameter name `expected_accuracy` is historical: the earlier v1.2 form
/// (`1 + 20 * (0.99204 - acc_expected)`) was deleted from the reference in v1.9b and no longer
/// exists, so the power-law loss form is the only behaviour to port. Pass
/// `spec().diagnostics.loss_anchor` / `loss_gamma` for `anchor` / `gain`.
///
/// A non-positive loss or anchor returns the lower clamp (a map that cannot be failed is not
/// rewarded), never a NaN.
